/*
 * cf1000-ledd.c - ORICO CF1000 NAS LED daemon
 * uburico-lights v1.0.0
 *
 * Monitors ZFS pool membership and SMART health for each drive bay,
 * then writes to the LED class devices created by orico_cf1000_leds.ko.
 *
 ...
 *
 * Config:  /etc/cf1000-led.conf  (INI format, never overwritten on update)
 * Usage:   cf1000-ledd [-f] [-c /path/to/config]
 *            -f   foreground mode (log to stderr instead of syslog)
 *            -c   config file path (default: /etc/cf1000-led.conf)
 */
...
#define DEFAULT_CONFIG      "/etc/cf1000-led.conf"
...
static const int SMART_WARN_IDS[] = { 5, 187, 197, 198, 199, -1 };
...
/*
 * Bay table — HCTL from board_config.txt, runtime fields populated at start.
 */
...
static config_t g_cfg = {
    ...
    .io_threshold          = 1,     /* sectors per tick; raise to reduce flicker */
...
static void log_msg(int priority, const char *fmt, ...)
{
    ...
    if (g_foreground) {
        ...
        fprintf(stderr, "[%s] cf1000-ledd: ", tag);
...
static int led_write(int bay, const char *color, int brightness)
{
    ...
    snprintf(path, sizeof(path),
             "%s/cf1000:bay%d:%s/brightness", SYSFS_LEDS, bay, color);
...
static bool leds_available(void)
{
    char path[256];
    snprintf(path, sizeof(path),
             "%s/cf1000:bay1:blue/brightness", SYSFS_LEDS);
    struct stat st;
...
/*
 * For a given HCTL (e.g. "5:0:0:0"), look for:
 *   /sys/class/scsi_device/5:0:0:0/device/block/<devname>/
...
static void refresh_network(void)
{
    ...
    char path[256];
    FILE *sf;
    snprintf(path, sizeof(path), "%s/cf1000:network:blue/brightness", SYSFS_LEDS);
    if ((sf = fopen(path, "w"))) { fprintf(sf, "%d\n", blue); fclose(sf); }
    snprintf(path, sizeof(path), "%s/cf1000:network:red/brightness", SYSFS_LEDS);
    if ((sf = fopen(path, "w"))) { fprintf(sf, "%d\n", red);  fclose(sf); }
}
...
static void refresh_system_led(void)
{
    ...
    char path[256];
    FILE *sf;
    snprintf(path, sizeof(path), "%s/cf1000:system:blue/brightness", SYSFS_LEDS);
    if ((sf = fopen(path, "w"))) { fprintf(sf, "%d\n", blue); fclose(sf); }
    snprintf(path, sizeof(path), "%s/cf1000:system:red/brightness", SYSFS_LEDS);
    if ((sf = fopen(path, "w"))) { fprintf(sf, "%d\n", red);  fclose(sf); }
}
...
int main(int argc, char *argv[])
{
    ...
    if (!g_foreground)
        openlog("cf1000-ledd", LOG_PID, LOG_DAEMON);

    log_info("cf1000-ledd starting");
    ...
    if (!leds_available()) {
        log_err("LED sysfs nodes not found — is orico_cf1000_leds.ko loaded?");
        log_err("Try: modprobe orico_cf1000_leds");
